...
**命令执行环境**：表格中所有命令（包括 Win 平台条目）均在 **WSL-1** 环境下执行，Windows 路径通过 `/mnt/c/` 访问。需要调用 Windows 原生命令时使用 `cmd.exe /c <命令>` 前缀。
...
| 🥉 | `wget.la` | ~450 KB/s | 中国 香港 |
| 4 | `ghfast.top` | ~2.8 MB/s | 韩国 首尔 |
| 5 | `ghproxy.net` | ~5.0 MB/s | 英国 |
...
| Snipaste-2.10.6-x64 | Win | <https://www.snipaste.com/download.html> | 2.11.2⁴ | `exiftool Snipaste.exe \| grep "Product Version"` | 见官网⁴ | ⏸️ |
